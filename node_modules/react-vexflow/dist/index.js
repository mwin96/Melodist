...
var clefAndTimeWidth = 60;

function Score(_ref) {
  var _ref$staves = _ref.staves,
      staves = _ref$staves === undefined ? [] : _ref$staves,
      _ref$clef = _ref.clef,
      clef = _ref$clef === undefined ? 'treble' : _ref$clef,
      _ref$timeSignature = _ref.timeSignature,
      timeSignature = _ref$timeSignature === undefined ? '4/4' : _ref$timeSignature,
      _ref$width = _ref.width,
      width = _ref$width === undefined ? 450 : _ref$width,
      _ref$height = _ref.height,
      height = _ref$height === undefined ? 150 : _ref$height,
      _ref$maxStavesX = _ref.maxStavesX,
      maxStavesX = _ref$maxStavesX === undefined ? Infinity : _ref$maxStavesX;
  ...
  React.useEffect(function () {
    ...
    renderer.resize(width, height * Math.ceil(staves.length / maxStavesX));
    var context = renderer.getContext();
    context.setFont('Arial', 10, '').setBackgroundFillStyle('#eed');
    var staveWidth = (width - clefAndTimeWidth) / Math.min(maxStavesX, staves.length);

    var currX = 0;
    var currY = 0;
    var renderedStaves = [];
    staves.map(function (notes, i) {
      if (i % maxStavesX === 0 && i !== 0) {
        currY += Math.ceil.apply(Math, toConsumableArray(renderedStaves.slice(i % maxStavesX * maxStavesX, i % maxStavesX * maxStavesX + maxStavesX).map(function (stave) {
          return stave.getHeight();
        })));
        currX = 0;
      }
      var stave = new Stave(currX, currY, staveWidth);
      ...
      var processedNotes = notes.map(function (note) {
        return typeof note === 'string' ? { key: note } : note;
      }).map(function (note) {
        return Array.isArray(note) ? { key: note[0], duration: note[1] } : note;
      }).map(function (note) {
        if (!note.key.includes('/')) {
          note.key = note.key[0] + '/' + note.key.slice(1);
        }
        if (note.key.includes('♯')) {
          note.key = note.key.replace('♯', '');
          note.sharp = true;
        }
        if (note.key.includes('♭')) {
          note.key = note.key.replace('♭', '');
        ...
        var key = _ref2.key,
            ...
            duration = _ref2$duration === undefined ? 'q' : _ref2$duration,
            sharp = _ref2.sharp,
            flat = _ref2.flat;
  ...
  return React__default.createElement('div', { ref: container });
}
